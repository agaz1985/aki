...
class TestKMeans(unittest.TestCase):
    def setUp(self):
        ...
        self._model = KMeans(self._device)
    ...
    def test_name(self):
        self.assertEqual("k-means", self._model.get_name())

    def test_save_load(self):
        # Fit and save a model.
        centroids, distance = self._model.fit(self._data,
                                              n_clusters=2,
                                              max_iterations=100,
                                              eps=1e-5)
        ...
        centroids_new, distance_new = new_model.predict(self._data)

        self.assertTrue(np.all(np.equal(centroids.detach().cpu().numpy(), centroids_new.detach().cpu().numpy())))
        self.assertTrue(np.all(np.equal(distance.detach().cpu().numpy(), distance_new.detach().cpu().numpy())))
