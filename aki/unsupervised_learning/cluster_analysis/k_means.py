...
class KMeans(ModelBase):
    ...
    def _init_parameters(self, input_data: torch.Tensor, n_clusters: int):
        ...
        self._c = input_data[torch.multinomial(input_data.T, self._n_clusters, replacement=False).T].squeeze(-1).to(
            self._device)

    def _expectation_step(self, x):
        repeated_input = x.unsqueeze(dim=1).repeat(1, self._c.shape[0], 1)
        self._distance_map = torch.norm(repeated_input - self._c.unsqueeze(dim=0), 2, dim=2)
